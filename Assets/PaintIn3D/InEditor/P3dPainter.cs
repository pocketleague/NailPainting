...
namespace PaintIn3D
{
	public partial class P3dPainter : P3dEditorWindow
	{
				...
				foreach (var obj in scene.Objs)
				{
					if (obj.Preview != null)
					{
						for (var i = 0; i < obj.MatIds.Count; i++)
						{
							...
							if (mat != null)
							{
								var mesh    = obj.FinalMesh;
								var submesh = Mathf.Min(mesh.subMeshCount - 1, i);

								Graphics.DrawMesh(mesh, obj.Preview.localToWorldMatrix, mat.Material, obj.Preview.gameObject.layer, null, submesh, mat.Properties);
							}
						}
					}
				}
					...
					if (obj.Mesh != null)
					{
						for (var i = 0; i < obj.MatIds.Count; i++)
						{
							...
							if (mat != null)
							{
								...
								previewUtil.DrawMesh(obj.FinalMesh, obj.Matrix, mat.Material, subMesh, mat.Properties, null, false);
							}
							else
							{
								previewUtil.DrawMesh(obj.FinalMesh, obj.Matrix, default(Material), subMesh, null, null, false);
							}
						}
					}
				}
		...
		private Ray GetRay()
		{
			...
			var s = Mathf.Max(1.0f, fullRect.height / fullRect.width);

			return previewUtil.camera.ViewportPointToRay(new Vector3(0.5f + u * s, 0.5f - v * s, 0.0f));
		}
		...
		private bool Raycast(Ray ray, ref RaycastHit hit, ref Mesh hitMesh, ref Matrix4x4 hitMatrix)
		{
			...
			if (scene != null)
			{
				foreach (var entity in scene.Objs)
				{
					...
					if (P3dWindowIntersect.IntersectRayMesh(ray, entity.FinalMesh, entity.Matrix, out tempHit) == true)
					{
						if (tempHit.distance < hit.distance)
						{
							hit       = tempHit;
							hitMesh   = entity.FinalMesh;
