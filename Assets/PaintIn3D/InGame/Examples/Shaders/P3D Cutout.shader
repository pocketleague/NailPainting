Shader "Paint in 3D/P3D Cutout"
{
	Properties
	{
		...
		[NoScaleOffset]_DetailTex("Detail Albedo (RGB) Detail Alpha (A)", 2D) = "white" {}
		_DetailTex_Transform("Scale (XY) Offset (ZW)", Vector) = (1.0, 1.0, 0.0, 0.0)
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			void surf(Input i, inout SurfaceOutputStandard o)
			{
				...
				float4 detail  = P3D_SAMPLE_PARALLAX(_DetailTex, i.coord, shift);

				o.Albedo     = texMain.rgb * _Color.rgb * detail.rgb;
				o.Normal     = UnpackScaleNormal(normal, _BumpScale);
				o.Metallic   = gloss.r * _Metallic;
				o.Occlusion  = gloss.g;
				o.Smoothness = gloss.b * _GlossMapScale;
				o.Alpha      = pow(texMain.a * _Color.a, 0.5f + detail.a * 1.0f);
			}
		ENDCG
	}
	FallBack "Standard"
