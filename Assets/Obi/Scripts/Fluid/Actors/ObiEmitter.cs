...
//            if ((m_IsEmitting || pooledParticles > Mathf.FloorToInt(minPoolSize * particleCount)) && SingletonClass.instance.IS_PRESSED)
                if ((m_IsEmitting || pooledParticles > Mathf.FloorToInt(minPoolSize * particleCount)))
            {
             //   Debug.Log("emitting "+SingletonClass.instance.IS_PRESSED);
                // stream emission:
